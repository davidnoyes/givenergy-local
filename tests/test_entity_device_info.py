"""Tests for device registry links between battery and inverter devices."""
...
def _battery_entity(
    ...
    entity.battery_id = 0
    entity._battery_serial_number = battery.serial_number
    entity._battery_bms_firmware_version = str(battery.bms_firmware_version)
    entity._battery_capacity_ah = battery.cap_design2
    if hass is not None:
        entity.hass = hass
    return entity
...
async def test_battery_links_to_registered_inverter(
    ...
    """The battery device references the inverter by registry ID, not identifier."""
